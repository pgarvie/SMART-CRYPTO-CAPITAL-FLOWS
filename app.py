...
def analyze_flows(df_t1: pd.DataFrame, df_t2: pd.DataFrame) -> pd.DataFrame:
    """Calcule la variation de market cap entre deux snapshots."""
    ...
    if "volume_T1" in merged.columns and "volume_T2" in merged.columns:
        vol_t1 = merged["volume_T1"].replace(0, np.nan)
        merged["Vol_Chg_Pct"] = (
            (merged["volume_T2"] - merged["volume_T1"]) / vol_t1 * 100
        ).round(2)
    else:
...
def compute_macro_crypto(df: pd.DataFrame, n_scanned: int) -> dict:
    ...
    n_pos   = int((df["Flux_M"] > 0).sum())
    n_neg   = int((df["Flux_M"] < 0).sum())
    ...
    has_vol = df["Vol_Chg_Pct"].notna().any()

    suspects_dist, suspects_pump = [], []
    if has_vol:
        suspects_dist = df[
...
def save_freeze(df: pd.DataFrame, macro: dict, label: str,
                t1: str, t2: str, interval_name: str, mode: str) -> None:
    """Persiste le freeze frame dans data/crypto_snapshot.json."""
    os.makedirs("data", exist_ok=True)
    payload = {
        "label": label, "t1": t1, "t2": t2,
        "interval_name": interval_name, "mode": mode,
...
# ══════════════════════════════════════════════════════════════════════════════
# GRAPHIQUES PLOTLY — retournent des Figure, n'appellent PAS st.*
# ══════════════════════════════════════════════════════════════════════════════

def make_inflow_chart(df: pd.DataFrame, title: str) -> go.Figure:
    """Graphique TOP 10 INFLOWS."""
    top = df.nlargest(10, "Flux_M").copy()
    fig = go.Figure(go.Bar(
        x=top["Flux_M"],
        y=top["name_col"],
        orientation="h",
        marker_color="#00c853",
        text=[f"+{v:.1f}M$" for v in top["Flux_M"]],
        textposition="outside",
        textfont=dict(size=9, color="#ccc"),
        hovertemplate=(
            "<b>%{y}</b><br>Inflow : +%{x:.2f} M$<br>"
            "Prix : %{customdata[0]:+.2f}%<extra></extra>"
        ),
        customdata=top[["Price_Chg_Pct"]].values,
    ))
    fig.update_layout(
        title=dict(text=title, font=dict(size=11, color="#00c853"), x=0.01),
...
def make_outflow_chart(df: pd.DataFrame, title: str) -> go.Figure:
    """Graphique TOP 10 OUTFLOWS."""
    top = df.nsmallest(10, "Flux_M").copy()
    fig = go.Figure(go.Bar(
        x=top["Flux_M"],
        y=top["name_col"],
        orientation="h",
        ...
        textfont=dict(size=9, color="#ccc"),
        hovertemplate=(
            "<b>%{y}</b><br>Outflow : %{x:.2f} M$<br>"
            "Prix : %{customdata[0]:+.2f}%<extra></extra>"
        ),
        customdata=top[["Price_Chg_Pct"]].values,
    ))
    fig.update_layout(
        title=dict(text=title, font=dict(size=11, color="#ff1744"), x=0.01),
        paper_bgcolor="#0e1117", plot_bgcolor="#0e1117",
        font=dict(color="#ccc", family="monospace"),
...
def make_balance_chart(macro: dict, title: str) -> go.Figure:
...
# ══════════════════════════════════════════════════════════════════════════════
# RENDU PANNEAU COMPLET — reçoit un container Streamlit explicite
# ══════════════════════════════════════════════════════════════════════════════

def render_top10_panel(container, df: pd.DataFrame,
                       title_prefix: str, panel_id: str) -> None:
    """
    Affiche les TOP 10 IN et OUT dans 'container'.
    Toutes les clés Streamlit sont suffixées par panel_id.
    """
    with container:
        sub_in, sub_out = st.columns(2)
        with sub_in:
            st.plotly_chart(
                make_inflow_chart(df, f"🟢 TOP 10 INFLOWS — {title_prefix}"),
                use_container_width=True,
                key=f"inflow_{panel_id}",
            )
        with sub_out:
            st.plotly_chart(
                make_outflow_chart(df, f"🔴 TOP 10 OUTFLOWS — {title_prefix}"),
                use_container_width=True,
                key=f"outflow_{panel_id}",
            )


def render_macro_panel(container, macro: dict, t1: str, t2: str,
                       interval_name: str, mode: str, panel_id: str) -> None:
    """
    Affiche le bloc macro complet dans 'container' :
    badge + note couverture + métriques + graphique balance + suspects.
    Tout est confiné dans le container reçu → aucun débordement possible.
    """
    with container:
        ...
        cc.metric("Flux Net",
                  f"{macro['total_flow']:+,.0f} M$")
        cd.metric("Actifs en hausse",
                  f"{macro['n_positive']}/{macro['n_scanned']}")
        ce.metric("Actifs en baisse",
                  f"{macro['n_negative']}/{macro['n_scanned']}")
        cf.metric("Score Santé",
                  f"{macro['health']:.2f}")

        # Graphique balance — key unique par panneau
        st.plotly_chart(
            make_balance_chart(
                macro,
            ...
            use_container_width=True,
            key=f"balance_{panel_id}",          # ← clé unique
        )
        ...
        if has_dist or has_pump:
            ...
            if has_pump:
                ...
                st.markdown(
                    f'<div class="suspects-box" '
                    f'style="border-left-color:#ff1744;">'
...
defaults = {
    ...
    "live_t1":           None,
    "live_t2":           None,
    "live_df_ref":       None,   # raw DataFrame T1 (snapshot de référence)
    "live_ts_ref":       None,   # datetime du T1
    "live_df_raw_prev":  None,   # raw DataFrame du cycle précédent (mode évolutif)
    # Freeze frame
    "freeze_df":         None,
    "freeze_macro":      None,
    "freeze_label":      None,
    ...
    # Mode par défaut : CUMULATIF (cohérent avec PLEXIS Sectoriel)
    "mode":              "Cumulatif (vs snapshot initial)",
}
...
with st.sidebar:
    ...
    mode = st.radio(
        "📈 Mode d'analyse",
        ["Cumulatif (vs snapshot initial)", "Évolutif (vs précédent)"],
        index=0 if st.session_state.mode == "Cumulatif (vs snapshot initial)" else 1,
    )
    ...
    if st.button("🔁 Réinitialiser T1 (nouveau snapshot de référence)",
        ...
        st.session_state.live_df_raw_prev = None
        st.session_state.live_df         = None
        st.session_state.live_macro      = None
        st.session_state.live_t1         = None
        st.session_state.live_t2         = None
    ...
    st.caption("Fige une copie du panneau LIVE à gauche pour comparaison.")
...
if time_since >= refresh_seconds:
    ...
    if df_new is not None:
        if st.session_state.live_df_ref is None:
            # ── Premier fetch : établissement du T1 ──────────────────────────
            ...
            # ── Fetches suivants : calcul des flux ────────────────────────────
            if cumulative:
                df_ref = st.session_state.live_df_ref
                t1_str = st.session_state.live_t1
            else:
                df_ref = st.session_state.live_df_raw_prev
                t1_str = st.session_state.live_t1
...
# Les deux fonctions render_*_panel reçoivent le container explicitement
# → zéro risque de rendu hors colonne
# ══════════════════════════════════════════════════════════════════════════════
...
with col_left:
    ...
    if st.session_state.freeze_df is not None:
        # Conteneur dédié pour les charts top10 du freeze
        freeze_top10_container = st.container()
        render_top10_panel(
            freeze_top10_container,
            st.session_state.freeze_df,
            f"{flabel} ({finter})",
            panel_id="frozen",
        )
    ...
    if st.session_state.freeze_macro is not None:
        # Conteneur dédié pour le macro du freeze — confine les suspects
        freeze_macro_container = st.container()
        render_macro_panel(
            freeze_macro_container,
            st.session_state.freeze_macro,
...
with col_right:
    ...
    if st.session_state.live_df is not None:
        # Conteneur dédié pour les charts top10 live
        live_top10_container = st.container()
        render_top10_panel(
            live_top10_container,
            st.session_state.live_df,
            f"{st.session_state.interval_name} · "
            f"{'CUM' if cumulative else 'ÉVO'}",
            panel_id="live",
        )
    elif st.session_state.live_df_ref is not None:
        st.info(
            f"⏳ T1 établi à **{lt1}** "
            f"({len(st.session_state.live_df_ref)} actifs chargés). "
    ...
    if st.session_state.live_macro is not None:
        # Conteneur dédié pour le macro live — confine les suspects
        live_macro_container = st.container()
        render_macro_panel(
            live_macro_container,
        ...
        st.caption(
            f"En attente du premier cycle complet "
            f"(T1 + {interval_minutes} min)..."
        )


# ══════════════════════════════════════════════════════════════════════════════
# BOUCLE DE POLLING — rerun toutes les 5 secondes
# ══════════════════════════════════════════════════════════════════════════════
